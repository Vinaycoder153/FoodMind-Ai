...
function renderOverview(insights) {
  ...
  sentimentChartInst = new Chart(sentimentCtx, {
    ...
    options: {
      ...
      plugins: {
        legend: { position: "bottom", labels: { color: "#8892aa", padding: 16, font: { size: 12 } } },
...
function renderReviewList(reviews) {
  ...
  reviews.forEach((r, idx) => {
    ...
    const tagHTML = (r.tags || [])
      .map((t) => `<span class="tag ${t.sentiment}">${featureIcon(t.feature)} ${t.feature}</span>`)
      .join("");
...
function renderRecommendations(recs) {
  ...
  if (!recs.length) {
    container.innerHTML = "<p style='color:var(--text-dim)'>No significant complaints detected. Keep up the great work! 🎉</p>";
    return;
  }
